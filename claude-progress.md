# Session 036: Editor UX — Trim (A) + Responsive Drawers (B-1) + SEO Settings (B-2) + Section Show/Hide (B-3)
**Status:** Verified / Passing
**Latest state:**
- UX audit of `Editor.tsx` vs PRD (form + template, not drag-and-drop). Trimmed dead/placeholder/discordant features, made the editor responsive on mobile, filled the right "Settings" tab with real SEO controls, and implemented per-section show/hide toggles.
- **A (trim)**: removed left icon dock (Database/Media/Global Settings placeholder panels), dead undo/redo header buttons (keyboard `Cmd/Ctrl+Z` via useHistory still works), duplicate dead device-switcher in header, fake pulsing "Live" indicator, dead `open_in_new` button, inline-editing overlay + its state/handlers (`handlePreviewDoubleClick`, `handleInlineSave`, `inlineEdit*` state), zoom simplified to `fit-screen | 50 | 100%`, removed unused `scaleFitHeight`/`availableH`.
- **B-1 (responsive)**: both the 300px left and 280px right sidebars behave as drawers below `lg` — fixed inset-y-0 left/right-0, translate slide, backdrop closes on tap, X close + toolbar toggles (`edit_note`/`tune`, `lg:hidden`), device switcher hides desktop/laptop below `sm`, header back-label + Preview/Save labels icon-only below `sm`.
- **B-2 (SEO)**: added optional `meta.seo` (`title`, `description`, `ogImage`) to `WebsiteDocument` in `src/templates/definition.ts`; Editor "Settings" tab renders a live search-result preview + Site Title/Description/Social Image URL fields; saved via existing autosave path (`documentForSave` now carries `seo`, `useAutosave` watches `{data, seo}`); `/sites/[subdomain]` `generateMetadata` + og/twitter now honor `meta.seo` (fallback ke profil).
- **B-3 (section show/hide)**: added `hiddenSections: z.array(z.string()).default([])` to `baseProfileSchema` + `hiddenSections: []` in `BASE_PROFILE_DEFAULTS` (`src/templates/shared/_base.ts`) — all 8 template schemas extend it. Added `hidden(id)` guards in every renderer (minimal, bold, corporate, creative, dark, freelancer, studio, portfolio-pro); portfolio-pro filters the arrays passed into `ResumeSection` (tabs self-hide) and its `Navbar` filters nav links for hidden sections. Editor `Sections` tab replaced its placeholder with per-section visibility toggle rows writing `data.hiddenSections`. Added `hiddenSections: []` to `freelancerDefaults` + `PREVIEW_DATA` (TemplateGallery/TemplateShowcase) for the now-required field.
