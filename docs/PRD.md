...
RLS mengikuti pola trigger proteksi field yang sudah ada di `20260811000008_designer_submissions.sql` — dipakai ulang, tidak ditulis ulang. `template_submissions`, bucket `template-submissions`, dan seluruh jalur upload ZIP pensiun setelah data draft dipindahkan (lihat Section 17.1 untuk status `designer-001` yang sudah dibangun di atas arsitektur lama).
...
- **2026-08-22**: audit arsitektur (Section 9A, dilebur dari `docs/PRD-v2.md`) mengoreksi mekanisme Designer Portal dari kode+ZIP+review-manual+deploy menjadi data (`layout_json` di database, section kit, `SectionRenderer` tunggal). **Ini mengunci arah arsitektur untuk Fase 2/3**, tapi **tidak mengunci nasib `designer-001`** — lihat 17.2, ini keputusan pemilik produk karena `designer-001` sudah berstatus *passing* dengan migration, RLS, storage privat, dan E2E teruji (bukan kerja kecil untuk dibuang tanpa persetujuan eksplisit).
...
- **Nasib `designer-001` yang sudah *passing* (Section 17.1, 22 Agustus 2026)**: Section 9A mengusulkan arsitektur berbeda (data-driven, bukan ZIP-upload). Tiga opsi konkret untuk pemilik produk pilih, bukan diputuskan otomatis oleh dokumen ini: (a) **pensiunkan** — hapus alur ZIP/bucket/migration terkait saat Fase 2 mulai, ganti total dengan Section 9A; kerja sunk-cost, tapi codebase bersih satu arsitektur; (b) **pertahankan sebagai jalur "advanced/legacy"** paralel dengan section kit untuk designer yang butuh kode kustom di luar cakupan section kit (Section 9A.9 eksplisit menaruh "kode/CSS kustom" di luar cakupan v2 — opsi ini membukanya kembali secara sengaja); (c) **pertahankan hanya metadata/lisensi/review-workflow-nya**, ganti isi kontennya dari ZIP jadi `layout_json` — migrasi parsial yang menyelamatkan skema review yang sudah ada di `20260811000008_designer_submissions.sql`. Rekomendasi kerja: (c), karena pola trigger proteksi field dan RLS-nya sudah teruji dan Section 9A.8 secara eksplisit menyebut akan dipakai ulang — tapi ini rekomendasi, bukan keputusan terkunci.
