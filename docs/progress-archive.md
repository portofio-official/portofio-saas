# Progress Log Archive — Sessions 001–015

Archived 2026-07-17 from `claude-progress.md` to keep the mandatory session-start read cheap (CLAUDE.md Operating Loop step 2). This covers the original MVP build (setup/auth/data/template/landing) before the Workspace Profile + Project System migration superseded the `PortfolioData`/`sites`/dummy-store architecture described in most of these entries. Historical reference only — not authoritative for current code shape; see `claude-progress.md`'s "Current Verified State" and `feature_list.json` for that.
